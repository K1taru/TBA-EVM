...
contract structsExampleDynamic {
  ...
  function uintToString(uint256 value) internal pure returns (string memory) { // strictly using only solidity
    if (value == 0) return "0";
    uint256 temp = value;
    uint256 digits;
    while (temp != 0) {
  ...
  function stringToGender(string memory sex) internal pure returns (Gender) { // beacause
  ...
  function genderToString(Gender sex) internal pure returns (string memory) { // why not
    if (sex == Gender.Male) {
      return "Male";
